...
require("snacks").setup({
    ...
    picker = {
        confirm = function(picker, item, action)
            local main_buf = picker.main and vim.api.nvim_win_get_buf(picker.main)
            local main_ft = main_buf
                and vim.api.nvim_get_option_value("filetype", { buf = main_buf })
                or nil
            local target = item or picker:current()
            local has_file = target and (target.file or target.buf)

            if main_ft == "dashboard" or not main_ft or not has_file then
                return actions.confirm(picker, item, action)
            end

            return actions.jump(picker, item, { cmd = "vsplit" })
        end,
...
map("<leader>fO", Snacks.picker.lsp_outgoing_calls, "Find outgoing calls")
